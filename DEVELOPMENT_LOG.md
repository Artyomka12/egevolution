...
**Ветка:** main
**Версия:** v1.3.0
**Статус:** готов к релизу — аудит безопасности и целостности данных завершён

Проект прошёл полный аудит безопасности (P0–P3, XSS, SQL, целостность данных).
Все найденные проблемы устранены. D-1, D-2, D-3 закрыты. Платформа работает стабильно.
...
**Завершён: аудит безопасности и целостности данных (v1.3.0, 2026-06-16)**

Все задачи аудита закрыты. Найденные в SQL-аудите проблемы D-1, D-2, D-3 устранены.

**SQL-аудит (итог):**
- Проверено: 72 вызова `db.execute()` (8 DDL + 64 DML/SELECT)
- SQL-инъекций: **0** — все запросы параметризованы (`?`-плейсхолдеры)
- `ORDER BY`, `LIMIT`, `OFFSET` — только статические имена колонок
- f-строки и конкатенация внутри SQL-строк: **отсутствуют**

**Целостность данных (итог):**

| # | Находка | Статус |
|---|---|---|
| D-1 | `delete_user()` не удалял из 5 связанных таблиц | ✅ Устранено (v1.2.6) |
| D-2 | `task_id` из JSON не валидировался как `int`, float/bool принимались | ✅ Устранено (v1.2.7) |
| D-3 | `str(e)` в двух admin-only маршрутах | ✅ Устранено (v1.2.7) |
...
| Rate limiting, MIME, CSP | ⏳ Отложено |

**Следующий цикл — на выбор:**
- **P3 Этап 2** — double-submit защита для `/save_results` и `/finish_exam`
- **Загрузка файлов** — проверка MIME-типа аватаров (сейчас только расширение)
- **CSP-заголовки** — Content-Security-Policy для защиты от XSS
- **Rate limiting** — ограничение попыток на `/login` и `/register`
