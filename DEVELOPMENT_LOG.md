...
**Ветка:** main
**Версия:** v1.5.3
**Статус:** выполнен — визуальная иерархия главной страницы, Этапы 1–3.3 (последняя проверка: 9/9 OK)
...
**Выполнено: визуальная иерархия главной страницы (v1.5.3, 2026-06-16)**
...
**Что сделано в v1.5.3:**
- stats-row: карточки → тонкая горизонтальная полоса с разделителями
- "С чего начать": равнозначный grid → `.start-flow` с `.start-arrow` (`→`/`↓`), левые акцентные бордеры
- Мини-задача: синий тинт + тег «Задание»; Калькулятор: фиолетовый тинт + тег «Калькулятор»
- Преимущества: 4 карточки → единая `.advantages-panel` с внутренними разделителями
- Feature Grid: 3 кол. → 2×2; карточка Статистика удалена; порядок — Подготовка / Теория / Варианты / База
- Контраст вторичного текста: 15 селекторов `#64748b` → `#94a3b8` (WCAG AA: 3.7:1 → 5.3:1)
- Interactive Zone: три интерактивных секции объединены в единую полосу (border-top/bottom + лёгкий фон)
- Backend не менялся; logout POST + CSRF сохранены; последняя проверка: **9/9 OK**
...
**Следующий шаг:** продолжение визуальной доработки `start.html` (Этап 4 из аудита) или наполнение базы задач для заданий 11–27
