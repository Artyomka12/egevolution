...
Завершён: **SQL-аудит (2026-06-16)** — 72 запроса проверены, инъекций не найдено.

**Результаты SQL-аудита:**
- Проверено: 72 вызова `db.execute()` (8 DDL + 64 DML/SELECT)
- SQL-инъекций: **0**
- Все пользовательские значения передаются через `?`-плейсхолдеры
- `ORDER BY`, `LIMIT`, `OFFSET` — только статические имена колонок, пользовательский ввод не попадает
- f-строки, конкатенация, `.format()` внутри SQL-строк: **отсутствуют**
- Уровень риска SQL-инъекций: **минимальный**

**Сопутствующие находки (не инъекции — целостность данных):**

| # | Находка | Место | Тип |
|---|---|---|---|
| D-1 | `delete_user()` не удаляет из 5 связанных таблиц | `app.py:2159–2161` | Целостность данных |
| D-2 | `task_id` из JSON не валидируется как `int` | `app.py:2046–2051` | Валидация входа |
| D-3 | `str(e)` в двух admin-only маршрутах | `app.py:1638, 2062` | Утечка ошибок |

Все три — низкий риск; D-3 ограничен `is_admin == 1`.
