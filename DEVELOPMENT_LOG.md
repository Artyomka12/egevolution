...
**Ветка:** main
**Версия:** v1.5.4
**Статус:** выполнен — последовательная интерактивная зона (Этап 2), последняя проверка: 24/24 OK
...
v1.5.4 — UX-доработка главной страницы: последовательная интерактивная зона (Этап 1: убран блок «Новые варианты», усилена кнопка navbar; Этап 2: пошаговый сценарий мини-задача → калькулятор → roadmap → CTA). Backend не менялся.
...
**Выполнено: последовательная интерактивная зона (v1.5.4, 2026-06-16)**
...
- Шаг 1 (мини-задача): результат → кнопка «Оценить свою готовность →»
- Шаг 2 (калькулятор): результат → кнопка «Посмотреть маршрут подготовки →»
- Шаг 3 (roadmap): маршрут подготовки ①–⑤ + CTA на бесплатное вводное занятие (Telegram + MAX)
- Переходы только по кнопкам — авто-переходов нет; `scrollIntoView` не используется
- Backend не менялся; logout POST + CSRF сохранены; последняя проверка: **24/24 OK**
