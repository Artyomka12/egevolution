...
**Ветка:** main
**Версия:** v1.10.2
**Статус:** выполнено — v1.10.0 (6 разноплановых доработок платформы за одну сессию: полная пересборка базы вариантов (20 новых сбалансированных вместо 14 старых), сквозная связь статистики попыток с базой заданий, статистика решаемости по каждому из 27 номеров заданий (для себя и в админке), открытие базы заданий и первого урока/темы теории для гостей, обязательное поле «Имя» и подсказка о пароле сайта на регистрации, версия сайта в футере, замена нечитаемых на мобильных эмодзи-замков на эмодзи+текст) + v1.10.1 (патч: исправлен баг списка тем теории — залогиненным пользователям без явного разрешения администратора тема №1 ошибочно показывалась закрытой) + v1.10.2 (редизайн ссылки на задание в разборе попытки — кнопка «Перейти к заданию» вместо эмодзи-ссылки; cache-busting `?v={{ app_version }}` на всей статике сайта — на живом сервере обнаружено 45-дневное кэширование CSS/JS без версионирования, из-за чего диаграмма статистики отображалась без стилей).
